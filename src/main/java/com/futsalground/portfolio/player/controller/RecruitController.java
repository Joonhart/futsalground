...
import com.futsalground.portfolio.player.model.*;
...
public class RecruitController {
    ...
    @GetMapping
    public String list(@ModelAttribute RecruitSearch recruitSearch, Model model, @Nullable String ajax, @PageableDefault(sort = "id", direction = Sort.Direction.DESC, size = 5)
            Pageable pageable) {
        Page<RecruitPageViewDto> recruitPageViewDtos = null;
        if (recruitSearch.getSearchCond() != null && recruitSearch.getSearchCond().equals("addr")) {
            recruitPageViewDtos = recruitService.findByAddr(pageable, recruitSearch.getSearchText());
        } else if (recruitSearch.getSearchCond() != null && recruitSearch.getSearchCond().equals("grdName")) {
            recruitPageViewDtos = recruitService.findByGrdName(pageable, recruitSearch.getSearchText());
        } else {
            recruitPageViewDtos = recruitService.findAll(pageable);
        }
        ...
        if (ajax != null) {
            return "player/recruitList :: #list";
        }
        return "player/recruitList";
    }

    @GetMapping("recrutingList")
    public String recrutingList(Model model, @PageableDefault(sort = "id", direction = Sort.Direction.DESC, size = 5)
            Pageable pageable) {
        Page<RecruitPageViewDto> recruitPageViewDtos = recruitService.findRecruting(pageable);
        int startPage = Math.max(1, recruitPageViewDtos.getPageable().getPageNumber() - 4);
        int endPage = Math.min(recruitPageViewDtos.getTotalPages(), recruitPageViewDtos.getPageable().getPageNumber() + 4);
        if (startPage > endPage) endPage = startPage;
        int curPage = recruitPageViewDtos.getPageable().getPageNumber() + 1;
